...
object KVAppService {
  ...
  def apply(system: ActorSystem, numNodes: Int, ackEach: Int): ActorRef = {
    ...
    val storesMeeting = for (i <- 0 until numNodes)
      yield system.actorOf(KVStore.props(), "GroupStoreMeeting" + i)

    /** Service tier: create app servers */
    val servers = for (i <- 0 until numNodes)
      yield system.actorOf(GroupServer.props(i, numNodes, storesMeeting, stores,  ackEach), "GroupServer" + i)
